...
export const TransactionList = ({ transactions }: TransactionListProps) => {
  ...
  const categories = Array.from(
    new Set(transactions.map((t) => t.to).filter(Boolean)),
  );

  const filtered = transactions.filter((t) => {
    if (search && !t.to.toLowerCase().includes(search.toLowerCase())) {
      return false;
    }

    if (selectedCategory && t.to !== selectedCategory) {
      return false;
    }
    ...
    const absPrice = Math.abs(t.value);
    ...
    <div className="w-full max-w-4xl mx-auto mt-6 px-4">
      ...
      {isFiltersOpen && (
        <div className="bg-[#D1FAE5] dark:bg-[#202024] border border-green-200 dark:border-[#323238] rounded-md p-5 mb-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 transition-all duration-300">
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-green-800 dark:text-[#7C7C8A]">
              Categoria
            </label>
            <select
      ...
      {hasActiveFilters && (
        <div className="flex flex-wrap gap-2 mb-4 items-center">
          <span className="text-xs text-[#7C7C8A] font-medium mr-1">
            Filtros ativos:
          </span>
          {selectedCategory && (
          ...
          {selectedType !== "all" && (
            <span className="bg-green-100 dark:bg-[#29292E] text-green-800 dark:text-[#00B37E] text-xs px-2.5 py-1 rounded-full flex items-center gap-1 border border-green-200 dark:border-[#323238]">
              Tipo: {selectedType === "Credit" ? "Entradas" : "Saídas"}
              <button
                onClick={() => setSelectedType("all")}
                className="hover:text-red-500 transition-colors cursor-pointer"
              >
